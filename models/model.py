...
def image_to_images(cluster, path, descriptions, models):
    dados = read_csv("C:/Users/eliab/Documents/projetao 2/cab.id/styles.csv")
    predict = run(path, descriptions, models)
    encoder = load("C:/Users/eliab/Documents/projetao 2/cab.id/models/encoder.pkl")
    print(predict)
    predict = DataFrame([predict])
    predict = encoder.transform(predict).toarray()
    print(predict)
    distancias, indices = cluster.kneighbors(predict)
